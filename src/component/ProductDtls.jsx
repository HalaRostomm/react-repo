...
const ProductDtls = ({ onProductUpdated }) => {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ color: '#14213D', fontWeight: 700, fontSize: 28, marginBottom: 8 }}>
        {product.productName}
      </h2>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
        {[...Array(5)].map((_, i) =>
          i < averageRating
            ? <FaStar key={i} color="#f5c518" size={22} />
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 280 }}>
          ...
          <p style={{ marginBottom: 8 }}>
            <strong>Sizes:</strong>{" "}
            <span style={{ color: '#555' }}>{[...new Set(Object.values(availableSizes).flat())].join(', ')}</span>
          </p>
      ...
      <button
        ...
        style={{
          backgroundColor: product.status === "available" ? "#14213D" : "#999",
          color: "white",
          padding: "14px 30px",
          border: "none",
          borderRadius: 8,
          ...
          boxShadow: product.status === "available" ? "0 4px 12px rgba(92,45,145,0.5)" : 'none',
          marginBottom: 40,
          width: '100%',
          maxWidth: 320,
          display: 'block',
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ marginBottom: 24, color: '#14213D', fontWeight: '700' }}>Select Options</h3>

            <label style={{ display: 'block', textAlign: 'left', marginBottom: 8, fontWeight: 600 }}>Color:</label>
            <select
              value={selectedColor}
              onChange={(e) => {
            ...
            {selectedColor && (
              <>
                <label style={{ display: 'block', textAlign: 'left', marginBottom: 8, fontWeight: 600 }}>Size:</label>
                <select
                  value={selectedSize}
                  onChange={(e) => {
            ...
            {selectedColor && selectedSize && (
              <>
                <p style={{ margin: '8px 0', fontWeight: 600 }}>
                  Stock: <span style={{ color: '#333' }}>{stockMap[`${selectedColor}-${selectedSize}`]}</span>
                </p>
                <p style={{ margin: '8px 0', fontWeight: 600 }}>
                  Price: <span style={{ color: '#333' }}>${(priceMap[`${selectedColor}-${selectedSize}`] || 0).toFixed(2)}</span>
                </p>
                <p style={{ margin: '8px 0', fontWeight: 600 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 14, marginTop: 14 }}>
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: quantity <= 1 ? '#eee' : '#5c2d91',
            ...
            <div style={{ marginTop: 32, display: 'flex', justifyContent: 'center', gap: 14 }}>
              ...
              <button
                ...
                style={{
                  backgroundColor: (!selectedColor || !selectedSize) ? '#999' : '#14213D',
                  border: 'none',
                  padding: '10px 26px',
                  borderRadius: 6,
                  fontWeight: '600',
      ...
      <div style={{ marginTop: 48 }}>
        <h3 style={{ color: '#14213D', fontWeight: 700, marginBottom: 24 }}>Write a Review</h3>
        <div style={{
          backgroundColor: '#fff',
          padding: 24,
          borderRadius: 12,
          ...
          <div style={{ display: 'flex', gap: 6, margin: '8px 0 18px', cursor: 'pointer' }}>
            {[...Array(5)].map((_, i) => (
              <span
                key={`input-star-${i}`}
                ...
                aria-label={`${i + 1} star`}
              >
                {i < userReview.rating ? <FaStar color="#f5c518" size={26} /> : <FaRegStar color="#ccc" size={26} />}
              </span>
            ))}
          </div>
        ...
        <h4 style={{ marginBottom: 20, fontWeight: 700, fontSize: 22, color: '#14213D' }}>All Reviews</h4>
        {reviews.length === 0 ? (
          ...
          reviews.map(rev => (
            ...
            >
              <div style={{ marginBottom: 6 }}>
                {[...Array(5)].map((_, j) =>
                  j < rev.rating
                    ? <FaStar key={j} color="#f5c518" size={18} />
 ...
 <small style={{ color: "#666" }}>
              ...
              <small style={{ color: '#777' }}>
                <strong>Date:</strong> {rev.date ? new Date(rev.date).toLocaleDateString() : "Unknown"}
              </small>
            </div>
          ))
        )}
